...
for netid in range(6, 7):
    for swipeid in range(2, 3):
        ...
        fd = open("download_bitrate_slow.txt", "w")
        # for entry in first_entries_parsed:
        #     tmp = 1

        for i in range(80):
            # fd.write(f"{quality_to_idx[first_entries_parsed[i][2]]}\n")
            fd.write(f"{first_entries_parsed[i][9]}\n")
        fd.close()
