...
def draw_ramp(ax, start, end):
    ax.add_patch(Rectangle((start, -2), end-start, 20, color=(0.96, 0.8, 0.8), zorder=0))

def draw_maintain(ax, start, end):
    ax.add_patch(Rectangle((start, -2), end-start, 20, color=(0.85, 0.92, 0.83), zorder=0))

def draw_idle(ax, start, end):
    ax.add_patch(Rectangle((start, -2), end-start, 20, color=(0.79, 0.85, 0.97), zorder=0))
...
for netid in range(10, 11):
    for swipeid in range(2, 3):
        ...
        t1 = first_entries_parsed[plot_range_low][0]
        t2 = first_entries_parsed[plot_range_low + 4][1]

        t3 = first_entries_parsed[plot_range_low + 9][1]

        t4 = first_entries_parsed[plot_range_low + 10][0]

        t5 = first_entries_parsed[plot_range_low + 17][1]

        t6 = first_entries_parsed[plot_range_low + 19][1]

        t7 = first_entries_parsed[plot_range_low + 20][0]

        play_string_arr = []

        dot_list = [31, 32, 35, 36, 38, 39, 40, 41, 43, 44, 45, 46, 47, 48, 49, 50]
        ...
        for i in range(plot_range_low, plot_range_high):
            ...
            print((i, first_entries_parsed[i][7] / 1000, swipe_log[i+1] - swipe_log[i]))

            # slides 5
            axs.plot([swipe_log[i], swipe_log[i+1]], [yaxis[i] % 10, yaxis[i] % 10], color="r", linestyle="-", linewidth=5)
            # slides 5
            if i in dot_list:
                scatter_x.append(swipe_log[i+1])
                scatter_y.append(yaxis[i] % 10)

            # slides 5
            if i != plot_range_low:
                axs.plot([swipe_log[i], swipe_log[i]], [yaxis[i-1] % 10, yaxis[i] % 10], color="r", linestyle="-", linewidth=5)
            # slides 5
        ...
        for i in range(plot_range_low, plot_range_high):
            ...
            if i in first_entries_parsed.keys():
                draw_bar(axs, i%10, first_entries_parsed[i][0], first_entries_parsed[i][1], quality_to_color(first_entries_parsed[i][2]), False)
            ...
            if i in second_entries_parsed.keys():
                axs.plot([first_entries_parsed[i][1], second_entries_parsed[i][0]], [(i% 10)+0.5, (i% 10)+0.5], color=quality_to_color((second_entries_parsed[i][2])), linestyle="-", linewidth=3)
                draw_bar(axs, i%10, second_entries_parsed[i][0], second_entries_parsed[i][1], quality_to_color((second_entries_parsed[i][2])), True)
        ...
        # slides 4
        axs.scatter(scatter_x, scatter_y, color="k", marker="o", s=300, zorder=100001, label="Swipe")
        # slides 4
        axs.bar(-20, 1, color=(181/255, 181/255, 181/255), label="Rebuffer")
        axs.bar(-20, 1, color=(0.96, 0.8, 0.8), label="Ramp-up state")
        axs.bar(-20, 1, color=(0.85, 0.92, 0.83), label="Maintaining state")
        axs.bar(-20, 1, color=(0.79, 0.85, 0.97), label="Prebuffer-idling state")
        axs.plot([-20, -21], [1, 1], color="r", linestyle="-", linewidth =5, label="Video Play")
        ...
        axs.legend(loc='upper left', bbox_to_anchor=(0, 1.45), ncol=5)
        ...
        axs.set_yticks([0.5, 4.5, 9.5])
        axs.set_yticklabels(["1", "5", "10"])

        axs.set_ylabel("Video index within group-of-10")
        ...
        axs.plot(idxs, buffer_arr, color="b", linewidth=3)
        ...
        axs.set_ylabel("# prebuffered videos")
        axs.set_xlabel("Time (s)")
