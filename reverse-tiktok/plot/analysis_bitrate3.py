    ...
    with open(foldername+play_log_name, newline='') as csvfile:
        ...
        for row in spamreader:
            ...
            for i in range(len(bitrate_names)):
                ...
                if bitrate_names[i] in norminal_bitrate.keys():
                    urireal_to_norminal[(row[1], bitrates[i])] = norminal_bitrate[bitrate_names[i]]

            for i in range(len(encode_uri)):
                uri_dict[encode_uri[i]] = (int(row[0]), int(i/2), len(bitrates))

                if bitrate_names[int(i/2)] in norminal_bitrate.keys():

                    uri_to_norminal[encode_uri[i]] = norminal_bitrate[bitrate_names[int(i/2)]]

                # uri_to_norminal[encode_uri[i]] = bitrate_names[int(i/2)]
    ...
    with open(foldername+download_log_name, newline='') as csvfile:
        ...
        for row in spamreader:
            ...
            if row[IDX_DOWNLOAD_URI] in uri_dict.keys():
                ...
                t_size = get_total_size(row[IDX_DOWNLOAD_RANGE])
                ...
                if row[IDX_DOWNLOAD_URI] in uri_to_norminal.keys():
                    bitrate_decision = uri_to_norminal[row[IDX_DOWNLOAD_URI]]
...
def analysis_entry2(foldername, play_log_name, download_log_name, swipe_log_name):
    ...
    with open(foldername+play_log_name, newline='') as csvfile:
        ...
        for row in spamreader:
            ...
            for i in range(len(encode_uri)):
                ...
                if (row[1], bitrates[int(i/2)]) in urireal_to_norminal:
                    norminalrate = urireal_to_norminal[(row[1], bitrates[int(i/2)])]

                    uri_to_norminal[encode_uri[i]] = norminalrate
...
for netid in range(6, 20):
    for swipeid in range(2, 7):
        ...
        bitrate_throughput_list, bitrate_buffer_list = analysis_entry2(foldername, play_log_name, download_log_name, swipe_log_name)
...
plt.figure()
ax = plt.imshow(table_absolute, cmap='rainbow')
