    ...
    with open(foldername+download_log_name, newline='') as csvfile:
        ...
        for row in spamreader:
            ...
            if row[IDX_DOWNLOAD_URI] in uri_dict.keys():
                ...
                # IDX_REQUEST_START
                if ranges[0] != 1024000:
                    first_entries.append([float(row[IDX_RESPONSE_START]), float(row[IDX_RESPONSE_END]), downloadIdx, uri_to_quality[row[IDX_DOWNLOAD_URI]], float(row[IDX_REQUEST_START]), throughput_est, size_in_bytes, total_size_in_bytes, chunk_duration])
                else:
                    second_entries.append([float(row[IDX_RESPONSE_START]), float(row[IDX_RESPONSE_END]), downloadIdx, uri_to_quality[row[IDX_DOWNLOAD_URI]], float(row[IDX_REQUEST_START]), throughput_est, size_in_bytes, total_size_in_bytes, chunk_duration])
        ...
        for i in range(plot_range_low, plot_range_high):
            ...
            if i in first_entries_parsed.keys():
                ...
                if first_entries_parsed[i][1] < swipe_log[i]:
                    # ax.fill_between([first_entries_parsed[i][1], swipe_log[i]], [first_bytes, first_bytes], color=cmaplist2[i%6], zorder=int(first_entries_parsed[i][0]))
                    data_dict[(i, 0)][1] = fill_buffer(first_entries_parsed[i][1], swipe_log[i], first_bytes, first_bytes, data_dict[(i, 0)][1], plot_bias)
                else:
