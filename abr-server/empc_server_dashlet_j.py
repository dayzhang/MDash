...
def make_request_handler(input_dict, logger, throughput_estimator):
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if post_data["run_schedule"] == -1:

                send_data= b"-2"
                self.write_response(send_data)
            ...
            # start schedule once next request come
            if post_data["run_schedule"] == 1:
                input_dict["run_schedule"] = True
                print(f"""{post_data["playerId"]}　- {post_data["chunkId"]}""")
                send_data= b"-2"
                self.write_response(send_data)
                return

            # conditionally run schedule
            ...
            # return -2 when initalization is not ready
            if post_data['currentPlayerIdx'] == -1:
                send_data = b"-2"
                self.write_response(send_data)
                return

            # convert requester Id
            ...
            allow_run = True
            for event in input_dict["events_record"]:
                if len(event) == 0:
                    allow_run = False

            if input_dict["run_schedule"] == True and allow_run == True:
                input_dict["run_schedule"] = False
