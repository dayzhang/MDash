...
class env:
    ...
    def loadSwipe(self, tracename, error):
        random.seed(0)
        ...
        for i in range(nlen):
            ...
            r_now = self.swipe_trace[i][0] * swipe_percentage[i]

            rmin = max(0, r_now - error)
            rmax = min(self.swipe_trace[i][0], r_now + error)

            val_random = random.uniform(rmin, rmax)

            self.swipe_trace[i][1] = val_random
    ...
    def getNextBuffer(
        ...
        for i in range(len(player_list)):
            ...
            ret = self.sendAbrRequest(player_list[i], i, current_player_idx, current_ts, 1, oracle_throughput)

        buffer_player = -1
        buffer_quality = -1
        for i in range(len(player_list)):

            ret = self.sendAbrRequest(player_list[i], i, current_player_idx, current_ts, 0, oracle_throughput)
            if ret != -2:
                buffer_player = i
                buffer_quality = ret
    ...
    def __init__(self, seqname, traceanme, networktracename, swipe_error):

        self.loadSequence(seqname)

        self.loadSwipe(traceanme, swipe_error)
    ...
    def sendAbrRequest(self, player, playerId, current_player_idx, current_ts, isinfo, oracle_throughput):
        next_chunk_size = []
...
def main(args):
    ...
    e = env(args.playtrace, args.swipetrace, args.networktrace, int(args.swipeerror))
    e.run()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--swipeerror', default=20, help='swipe error')
