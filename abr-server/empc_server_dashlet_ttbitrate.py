...
def get_bitrate2(events):
    ...
    for eidx, event in enumerate(events):
        ...
        # for i in range(len(event['chunksize_list'])):
        #     bitrate_list[eidx].append([])
        clen = min(5, event['duration'])
        for j in range(len(event['chunksize_list'][0])):
            bitrate_list[eidx].append(event['chunksize_list'][0][j] * 8 / clen) # in kb
...
class ThroughputEstimator:
    ...
    def get_throughput_est(self):
        ...
        history_start = max(history_len - 1, 0)
...
def determine_bitrate(throughput, bitrate_list):

    target_bitrate = 150858.48070105485 + 93874.02577906292 * throughput
                ...
                if post_data['Type'] == 'swipe':
                    ...
                    if input_dict["info_phase"] == True:
                        ...
                        if self.input_dict["ITER_CNT"] < len(buffer_test):
                            ...
                            for ki in range(len(input_dict["buffer_plan"])):
                                if input_dict["buffer_plan"][ki] != -2:
                                    bitrate_list2 = get_bitrate2(input_dict["events_record"])
                                    input_dict["buffer_plan"][ki] = determine_bitrate(throughput / 1000, bitrate_list2[ki])
...
if __name__ == "__main__":
    try:
        ...
        # parser.add_argument('--probabilitytraces', default='./probability/user-study/',
        #                     help='The probability trace')
        parser.add_argument('--probabilitytraces', default='./probability/lamda5/', help='The probability trace')
