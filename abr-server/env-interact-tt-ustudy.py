...
class env:
    ...
    def getbitrate(self, throughput, bitrate_list):
        ...
        target_bitrate = 150858.48070105485 + 93874.02577906292 * throughput[-1]
    ...
    def getNextBuffer(
        ...
        if player_list[current_player_idx].last_request_idx + 1 < len(
                player_list[current_player_idx].vsize[player_list[current_player_idx].last_quality]):
            return (current_player_idx, player_list[current_player_idx].last_request_idx + 1,
                    player_list[current_player_idx].last_quality)
    ...
    def timeToDownload(self, filesize, real_time):
        ...
        n_packets = int(filesize / 1500.0 / 1.0)

        if self.time_idx + n_packets >= len(self.networktrace):
            time_in_ms = self.networktrace[self.time_idx + n_packets - len(self.networktrace)] + self.networktrace[
                len(self.networktrace) - 1] - self.networktrace[self.time_idx]
        else:
        ...
        while view_vidx <= threshould:
            ...
            if (view_vidx, view_cidx) not in download_time_dict.keys():
                ...
                if len(unfinished_record_play) == 0:
                    time_to_view_finish = player_list[view_vidx % 5].segtime[player_list[view_vidx % 5].last_quality][
                        view_cidx]
                else:
