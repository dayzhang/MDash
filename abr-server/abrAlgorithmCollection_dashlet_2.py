...
def parse_buffer_status(events):
    ...
    current_cursor = min(buffer_length[0] * 2, video_duration[0]) - events[0]["buffer"]
...
def dash_sv(events, probability_weights, bitrate_profile, estimate_throughput):
    ...
    total_lengths = [int((vduration - 0.00000001)/ 2.0) + 1 for vduration in video_duration]

    cursor_idx = int(current_cursor / 2.0) + 1
...
def oracle_sv(events, probability_weights, bitrate_profile, throughput_estimate):
    ...
    for bitrate_choice in bitrate_choices:
        ...
        for i in range(len(download_list)):
            rebuffer = max(0, download_time[i] + 2 - (watch_start_time[download_list[i]] + total_rebuffer))
