...
def dash_sv2(events, probability_weights, bitrate_profile, estimate_throughput):
    stime = time.time()
...
def oracle_sv(events, probability_weights, bitrate_profile, throughput_estimate):
    ...
    for bitrate_choice in bitrate_choices:
        ...
        for i in range(len(download_list)):
            rebuffer = max(0, download_time[i] + 5 - (watch_start_time[download_list[i]] + total_rebuffer))
...
def tt_order(events, probability_weights, bitrate_profile, estimate_throughput):
    stime = time.time()
    ...
    if buffer_length[0] < total_lengths[0]:
        max_buffer_i = 0
    else:
        for i in range(1, len(buffer_length)):
...
def dash_mpc(events, probability_weights, bitrate_profile, estimate_throughput):
    stime = time.time()
    ...
    candidate_num = candidate_high[0] - buffer_length[0]
