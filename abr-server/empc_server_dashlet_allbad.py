...
from abrAlgorithmCollection_dashlet import tt_order
                ...
                if post_data['Type'] == 'swipe':
                    ...
                    if input_dict["info_phase"] == True:
                        ...
                        if self.input_dict["ITER_CNT"] < len(buffer_test):
                            ...
                            cp_idx = input_dict["events_record"][0]["currentPlayerIdx"]

                            input_dict["buffer_plan"] = tt_order(input_dict["events_record"], probability_weights, bitrate, throughput)

                            for i in range(len(input_dict["buffer_plan"])):
                                if input_dict["buffer_plan"][i] != -2:
                                    if input_dict["events_record"][i]["lastquality"] != -1:
                                        input_dict["buffer_plan"][i] = input_dict["events_record"][i]["lastquality"]

                            for p_idx in range(len(input_dict["events_record"])):
                                if input_dict["events_record"][cp_idx]["realPlayerId"] // 10 != input_dict["events_record"][p_idx]["realPlayerId"] // 10:
                                    if input_dict["events_record"][p_idx]["realPlayerId"] - input_dict["events_record"][cp_idx]["realPlayerId"] > 2:
                                        input_dict["buffer_plan"][p_idx] = -2
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument('--probabilitytraces', default='./probability/lamda5/', help='The path to save processed data')
