...
def tochunklength(arr):

    nlen = len(arr)

    olen = int((nlen + 4) / 5)

    ret = [0.0 for i in range(olen)]
...
def calculate(arr_a, arr_b):
    nomalize_a = tochunklength(arr_a)
    nomalize_b = tochunklength(arr_b)

    kl_ab = kl_divergence(nomalize_a, nomalize_b)
    kl_ba = kl_divergence(nomalize_b, nomalize_a)
...
class env:
    ...
    def try_abr(
            self, player_list, current_player_idx, last_buffer_player_idx, current_ts, probability_should
    ):
        ...
        for i in range(len(player_list)):
            ...
            ret = self.sendAbrRequest(player_list[i], i, current_player_idx, current_ts, 1, oracle_throughput, probability_should[i])

        buffer_player = -1
        buffer_quality = -1

        for i in range(len(player_list)):
            ret = self.sendAbrRequest(player_list[i], i, current_player_idx, current_ts, 0, oracle_throughput, probability_should[i])
    ...
    def getNextBuffer(
        ...
        for i in range(len(player_list)):
            prob_list = list(probability_map[player_list[i].uid])
            probability_should.append(prob_list)

        ret = self.try_abr(player_list, current_player_idx, last_buffer_player_idx, current_ts, probability_should)

        ret_loc = ret[0]

        rand_prediction_record = []
        for rand_id in range(1, 10):

            rand_prediction_record_inner = {}

            rand_prediction_record_inner["prob"] = []

            for i in range(len(player_list)):
                rand_prediction_record_inner["prob"].append(get_swipe_dis_by_id(player_list[i].duration, rand_id / 10))

            ret_this = self.try_abr(player_list, current_player_idx, last_buffer_player_idx, current_ts, rand_prediction_record_inner["prob"])

            rand_prediction_record_inner["loc"] = ret_this[0]

            rand_prediction_record.append(rand_prediction_record_inner)

            self.logfd.write(f"{ret_this[0]} ")


        # current_play_chunk_id

        playerhead = ret[0] - current_player_idx
        if playerhead < 0:
            playerhead += 5

        # print(f"{playerhead} {ret[1] - player_list[ret[0]].current_play_chunk_id}\n")
        self.logfd.write(f"{ret[0]} {playerhead} {ret[1] - player_list[ret[0]].current_play_chunk_id}\n")
        self.logfd.flush()

            # if ret_this[0] != ret[0]:
            #     print("=============")
            #
            #     for dist_i in range(max(ret_this[0], ret[0]) + 1):
            #         kl = calculate(rand_prediction_record_inner["prob"][dist_i], probability_should[dist_i])
            #
            #         print(kl)
            #
            #     print("=============")
    ...
    def init_throughput(self, trace):
        ...
        for i in range(len(self.network_throughput)):
            self.network_throughput[i] = self.network_throughput[i] * 1500 * 8 / 1000

    def __init__(self, seqname, traceanme, networktracename, logtrace):

        self.loadSequence(seqname)

        self.logfd = open(logtrace, "w")
    ...
    def sendAbrRequest(self, player, playerId, current_player_idx, current_ts, isinfo, oracle_throughput, probability):
        next_chunk_size = []
...
def main(args):
    ...
    e = env(args.playtrace, args.swipetrace, args.networktrace, args.logtrace)
    e.run()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--swipetrace', default="/home/acer/Documents/reverse-tiktok/traces/swipe/swipe-4.txt", help='Viewing trace')
    parser.add_argument('--networktrace', default="/home/acer/Documents/reverse-tiktok/traces/network/trace-5.0.down", help='The network trace')
    parser.add_argument('--probabilitytraces', default='./probability/lamda4/', help='The path to save processed data')
    parser.add_argument('--logtrace', default='./stable.log', help='The path to save processed data')
