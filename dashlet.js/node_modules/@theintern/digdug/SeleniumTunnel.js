...
var webdriversJson = tslib_1.__importStar(require("./webdrivers.json"));
    ...
    SeleniumTunnel.prototype.download = function (forceDownload) {
        ...
        return this._updateWebDriverData().then(function () {
            ...
            return new common_1.Task(function (resolve) {
                var configs = tslib_1.__spreadArray([
                    {
                        url: _this.url,
                        executable: _this.artifact,
                        dontExtract: true,
                    }
                ], _this._getDriverConfigs());
...
function versionGreaterThan(versionStr, version) {
    var parts = String(versionStr).split('.').map(Number);
...
var ChromeConfig = (function (_super) {
    ...
    function ChromeConfig(options) {
        ...
        var useNewDriverLocation = versionGreaterThan(opts.version, [114]);
        if (useNewDriverLocation && opts.baseUrl == webdrivers.chrome.baseUrl) {
            opts.baseUrl =
                'https://edgedl.me.gvt1.com/edgedl/chrome/chrome-for-testing';
        }
        _this = _super.call(this, Object.assign(opts, options)) || this;
        _this.useNewDriverLocation = useNewDriverLocation;
        return _this;
    }
    Object.defineProperty(ChromeConfig.prototype, "artifact", {
        get: function () {
            ...
            if (platform === 'linux') {
                ...
                var parts_1 = String(this.version).split('.').map(Number);
            ...
            return this.useNewDriverLocation
                ? util_1.format('chromedriver-%s.zip', platform)
                : util_1.format('chromedriver_%s.zip', platform);
        },
        enumerable: false,
        configurable: true
    });
    ...
    Object.defineProperty(ChromeConfig.prototype, "url", {
        get: function () {
            return this.useNewDriverLocation
                ? util_1.format('%s/%s/%s/%s', this.baseUrl, this.version, this.platform, this.artifact)
                : util_1.format('%s/%s/%s', this.baseUrl, this.version, this.artifact);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(ChromeConfig.prototype, "executable", {
        get: function () {
            return this.useNewDriverLocation
                ? path_1.join(this.directory, util_1.format('chromedriver-%s', this.platform), this.platform === 'win32' || this.platform === 'win64'
                    ? 'chromedriver.exe'
                    : 'chromedriver')
                : path_1.join(this.directory, this.platform === 'win32' ? 'chromedriver.exe' : 'chromedriver');
        },
        enumerable: false,
        configurable: true
    });
